...
import {
  ...
  MAXIMUM_LENGTH_OF_TYPING_USERS,
} from '../constants';
...
const mapStateToProps = (state: State) => ({
  generateTypingIndicatorList: () => {
    ...
    let typingUsers = typingUsersFromStore.filter(
      (typingUser: any) =>
        typingUser.user.communicationUserId !== currentContosoUser.identity
    );
    if (
      typingUsers.length === 0 ||
      state.threadMembers.threadMembers.length >= PARTICIPANTS_THRESHOLD
    ) {
    ...
    if (typingUsers.length > 0) {
      typingIndicator += typingUsers
        .filter(
          (typingUser: any) =>
            contosoUsers[typingUser.user.communicationUserId] !== undefined
        )
        .slice(0, 2)
        .map(
          (typingUserWithEmoji: any) =>
            `${
              contosoUsers[typingUserWithEmoji.user.communicationUserId].emoji
            }${typingUserWithEmoji.displayName}`
        )
        .join(', ');
    }
    ...
    if (typingIndicator.length > MAXIMUM_LENGTH_OF_TYPING_USERS) {
      typingIndicator = `${typingUsers.length} participant${
