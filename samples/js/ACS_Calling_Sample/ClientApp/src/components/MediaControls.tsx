...
import { MicIcon, MicOffIcon, CallVideoIcon, CallVideoOffIcon, CallEndIcon, CallControlPresentNewIcon, CallControlCloseTrayIcon } from '@fluentui/react-icons-northstar';
...
export default function MediaControls(props: MediaControlsProps): JSX.Element {
    const cameraActive = props.cameraPermission == "Denied" ? false : props.cameraActive;
    const cameraDisabled = props.cameraPermission == "Denied" || props.localVideoRendererIsBusy;
    ...
    return (
        <Stack className={mediaControlStyles}>        
            <CommandButton disabled={cameraDisabled} onClick={cameraOnClick} className={ controlButtonStyle }>
            ...
            <CommandButton onClick={micOnClick} disabled={micDisabled} className={ controlButtonStyle }>
                <div className={fullWidth}>
                    {micActive ? <MicIcon size="medium" /> : <MicOffIcon size="medium" />}
                </div>
            </CommandButton>
            {
                props.isLocalScreenShareSupportedInBrowser() &&
                <CommandButton disabled={screenShareDisabled} onClick={props.onScreenShareChange} className={ screenShareDisabled ? controlButtonDisabledStyle : controlButtonStyle }>
                <div className={fullWidth}>
                    {props.screenShareActive ? <CallControlCloseTrayIcon size="medium" /> : <CallControlPresentNewIcon size="medium" />}
                </div>
                </CommandButton>
            }
            <CommandButton onClick={props.onEndCallClick} className={props.compressedMode ? controlButtonStyle : endCallButtonStyle}>
                <div className={fullWidth}>
                    <CallEndIcon size="medium" />
                    { !props.compressedMode && 
                        <span className={endCallButtonTextStyle}>Leave</span> 
                    }
                </div>
            </CommandButton>
        </Stack>
